  ...
  methods: {
    ...
    getData () {
      ...
      fetchData(this.query).then(res => {
        // 将 数据中的 list 数组赋给 tableData 数组。
        this.tableData = res.list;
        // 将数据中的总条目数经过判断后赋给 data 中的 pageTotal
        this.pageTotal = res.pageTotal || 50;
      });
    },
    // 触发搜索按钮
    handleSearch () {
      // 为 this.query 新增属性，并更新视图。
    ...
    handleSelectionChange (val) {
      // 将选中数据的集合赋给 this.multipleSelection
    ...
    delAllSelection () {
      ...
      if (length) {
        let str = '';
        // 将选中的选项中的数据集合与 delList 数组连接起来
        this.delList = this.delList.concat(this.multipleSelection);
        // 对选中的选项进行循环，获取其中的 name 值
